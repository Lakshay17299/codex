...
public class RagService {
    ...
    private String answerFact(String question) {
        String cleaned = question.replace("?", "").trim().toLowerCase();

        // gather the verbs we know about from the graph relations
        java.util.Set<String> verbs = new java.util.HashSet<>();
        for (GraphRelation r : relationRepository.findAll()) {
            verbs.add(r.getType());
        }

        for (String verb : verbs) {
            java.util.regex.Matcher m = java.util.regex.Pattern
                    .compile(".*\\b" + java.util.regex.Pattern.quote(verb) + "\\b\\s+(.+)", java.util.regex.Pattern.CASE_INSENSITIVE)
                    .matcher(cleaned);
            if (m.matches()) {
